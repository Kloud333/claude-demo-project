...
Демонстраційний Node.js-проєкт — навчальний приклад, щоб показати
структуру `.claude/` в дії. Реального Express API тут немає (це не
production-код), лише один простий файл `src/example.js` для
реалістичності.
...
<!--
  ⚠️ Правила нижче — ІЛЮСТРАТИВНИЙ приклад того, як виглядають конкретні,
  перевірювані інструкції в реальному проєкті. Цей крихітний демо-репо
  не має папки src/api/handlers — приклад показує ФОРМАТ хорошого
  правила, не буквальну структуру саме цього репо.
